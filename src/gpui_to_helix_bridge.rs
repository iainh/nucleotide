...
pub fn send_gpui_event_to_helix(event: GpuiToHelixEvent) {
    if let Some(sender) = GPUI_TO_HELIX_SENDER.get() {
        ...
        log::warn!("GPUI->Helix event bridge not initialized, dropping event: {:?}", event);
    }
}
...
pub fn create_gpui_to_helix_channel() -> (mpsc::UnboundedSender<GpuiToHelixEvent>, GpuiToHelixEventReceiver) {
...
pub fn handle_gpui_event_in_helix(event: &GpuiToHelixEvent, editor: &mut helix_view::Editor) {
    match event {
        ...
        GpuiToHelixEvent::ExternalFileChanged { doc_id, path } => {
            log::info!("External file change detected: {:?} for doc {:?}", path, doc_id);
        ...
        GpuiToHelixEvent::AccessibilityChanged { high_contrast, screen_reader } => {
            log::info!("Accessibility changed: high_contrast={}, screen_reader={}", high_contrast, screen_reader);
