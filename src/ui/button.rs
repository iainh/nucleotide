    ...
    fn render(self, _window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        button = match self.variant {
            ButtonVariant::Primary => button
                .bg(if self.disabled { hsla(theme.accent.h, theme.accent.s, theme.accent.l, 0.5) } else { theme.accent })
                .text_color(white())
            ...
            ButtonVariant::Secondary => button
                .bg(if self.disabled { hsla(theme.surface.h, theme.surface.s, theme.surface.l, 0.5) } else { theme.surface })
                .text_color(if self.disabled { theme.text_disabled } else { theme.text })
                .border_1()
                .border_color(if self.disabled { hsla(theme.border.h, theme.border.s, theme.border.l, 0.5) } else { theme.border })
                .when(!self.disabled, |this| {
            ...
            ButtonVariant::Danger => button
                .bg(if self.disabled { hsla(theme.error.h, theme.error.s, theme.error.l, 0.5) } else { theme.error })
                .text_color(white())
                .when(!self.disabled, |this| {
