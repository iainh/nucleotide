...
impl Workspace {
    ...
    fn handle_editor_event(&mut self, ev: &helix_view::editor::EditorEvent, cx: &mut Context<Self>) {
    ...
    fn handle_selection_changed(&mut self, doc_id: helix_view::DocumentId, view_id: helix_view::ViewId, cx: &mut Context<Self>) {
    ...
    fn handle_mode_changed(&mut self, old_mode: &helix_view::document::Mode, new_mode: &helix_view::document::Mode, cx: &mut Context<Self>) {
    ...
    fn handle_diagnostics_changed(&mut self, doc_id: helix_view::DocumentId, cx: &mut Context<Self>) {
    ...
    fn handle_language_server_initialized(&mut self, server_id: helix_lsp::LanguageServerId, cx: &mut Context<Self>) {
        // LSP server initialized - update status
        info!("Language server initialized: {:?}", server_id);
        cx.notify();
    }
    
    fn handle_language_server_exited(&mut self, server_id: helix_lsp::LanguageServerId, cx: &mut Context<Self>) {
        // LSP server exited - update status
        info!("Language server exited: {:?}", server_id);
        cx.notify();
    }
    
    fn handle_completion_requested(&mut self, doc_id: helix_view::DocumentId, view_id: helix_view::ViewId, trigger: &crate::event_bridge::CompletionTrigger, cx: &mut Context<Self>) {
    ...
    fn execute_typed_command(&mut self, command: crate::command_system::Command, cx: &mut Context<Self>) {
        ...
        match command {
            ...
            Command::Replace { pattern, replacement } => {
    ...
    fn handle_open_directory(&mut self, path: &std::path::Path, cx: &mut Context<Self>) {
        ...
        let new_file_tree = cx.new(|cx| {
            let config = FileTreeConfig::default();
            FileTreeView::new(path.to_path_buf(), config, cx)
        });
    ...
    pub fn handle_event(&mut self, ev: &crate::Update, cx: &mut Context<Self>) {
        ...
        match ev {
            ...
            crate::Update::SelectionChanged { doc_id, view_id } => self.handle_selection_changed(*doc_id, *view_id, cx),
            crate::Update::ModeChanged { old_mode, new_mode } => self.handle_mode_changed(old_mode, new_mode, cx),
            crate::Update::DiagnosticsChanged { doc_id } => self.handle_diagnostics_changed(*doc_id, cx),
            ...
            crate::Update::LanguageServerInitialized { server_id } => self.handle_language_server_initialized(*server_id, cx),
            crate::Update::LanguageServerExited { server_id } => self.handle_language_server_exited(*server_id, cx),
            crate::Update::CompletionRequested { doc_id, view_id, trigger } => self.handle_completion_requested(*doc_id, *view_id, trigger, cx),
        }
    }
...
impl Render for Workspace {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let handle = self.handle.clone();
        let core = self.core.clone();
        workspace_div = workspace_div.on_action(cx.listener(move |_, _: &crate::actions::editor::Quit, _window, cx| {
        ...
        let handle = self.handle.clone();
        let core = self.core.clone();
        workspace_div = workspace_div.on_action(cx.listener(move |_, _: &crate::actions::editor::OpenFile, _window, cx| {
            open(core.clone(), handle.clone(), cx)
        }));
        
        let handle = self.handle.clone();
        let core = self.core.clone();
        workspace_div = workspace_div.on_action(cx.listener(move |_, _: &crate::actions::editor::OpenDirectory, _window, cx| {
        ...
        let handle = self.handle.clone();
        let core = self.core.clone();
        workspace_div = workspace_div.on_action(cx.listener(move |_, _: &crate::actions::workspace::ShowBufferPicker, _window, cx| {
        ...
        let handle = self.handle.clone();
        let core = self.core.clone();
        workspace_div = workspace_div.on_action(cx.listener(move |_, _: &crate::actions::help::OpenTutorial, _window, cx| {
            load_tutor(core.clone(), handle.clone(), cx)
        }));
        
        let handle = self.handle.clone();
        let core = self.core.clone();
        workspace_div = workspace_div.on_action(cx.listener(move |_, _: &crate::actions::test::TestPrompt, _window, cx| {
            test_prompt(core.clone(), handle.clone(), cx)
        }));
        
        let handle = self.handle.clone();
        let core = self.core.clone();
        workspace_div = workspace_div.on_action(cx.listener(move |_, _: &crate::actions::test::TestCompletion, _window, cx| {
...
fn show_buffer_picker(core: Entity<Core>, _handle: tokio::runtime::Handle, cx: &mut App) {
    ...
    let current_doc_id = core.read(cx).editor.tree.get(core.read(cx).editor.tree.focus).doc;
    ...
    let buffer_picker = crate::picker::Picker::native(
        "Switch Buffer",
        items,
        move |index| {
