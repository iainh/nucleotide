...
- SSH and WSL helper deployment can auto-upload or remotely download Linux `nucleotide-remote` helpers for `x86_64` and `aarch64`. Both transports probe the target platform, use a versioned XDG cache path, verify protocol compatibility, and retry once with a fresh helper after a startup compatibility failure.
...
| Helper deployment | Local projects need no helper binary. | SSH and WSL install bundled Linux helpers by default, with checksum-verified remote download as a fallback. | There are no non-Linux remote helper targets, signatures beyond release checksums, or an explicit compatibility policy for older helpers beyond protocol/frame/version validation. |
...
| Tests and fixtures | Local backend behaviour has many direct unit and integration tests. | Remote behaviour has loopback and protocol tests, focused UI-path tests, and fake SSH/WSL process fixtures that exercise helper upload on both transports. | We still need higher-level tests against real SSH and WSL targets for file tree polling, LSP, VCS, search, terminals and deployment edge cases. |
