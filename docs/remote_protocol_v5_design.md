...
- Replace whole-result accumulation for search and process output with bounded incremental consumers. File reads already use this model.
- Tie search and process `WINDOW_UPDATE` emission to incremental consumer capacity, matching file reads.
- Give the new incremental search and process APIs explicit cancellation tokens.
