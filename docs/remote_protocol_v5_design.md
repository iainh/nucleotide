...
- Replace whole-result accumulation for process output with a bounded incremental consumer. File reads and searches already use this model.
- Tie process `WINDOW_UPDATE` emission to incremental consumer capacity, matching file reads and searches.
- Give the new incremental process API an explicit cancellation token. Search streams already retain their explicit token registrations until completion or drop.
