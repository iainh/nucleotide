...
/// Defaults that apply to all completion items in a completion list.
///
/// @since 3.17.0
#[derive(Debug, PartialEq, Clone, Default, Deserialize, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct CompletionListItemDefaults {
    /// A default edit range that applies when an item does not specify
    /// `textEdit`.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub edit_range: Option<CompletionListItemDefaultsEditRange>,

    /// A default set of commit characters.
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    /// A default data value preserved for completion item resolve.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub data: Option<Value>,
}

/// The edit range shape used by `CompletionList.itemDefaults`.
///
/// @since 3.17.0
#[derive(Debug, Eq, PartialEq, Clone, Deserialize, Serialize)]
#[serde(untagged)]
...
impl<'de> Deserialize<'de> for CompletionList {
        ...
        D: serde::Deserializer<'de>,
    {
        #[derive(Deserialize)]
        #[serde(rename_all = "camelCase")]
        struct CompletionListHelper {
            is_incomplete: bool,
            #[serde(default)]
            item_defaults: Option<CompletionListItemDefaults>,
            items: Vec<CompletionItemWithTextEditText>,
        }

        #[derive(Deserialize)]
        #[serde(rename_all = "camelCase")]
        struct CompletionItemWithTextEditText {
            #[serde(flatten)]
            item: CompletionItem,
            #[serde(default)]
        ...
        let helper = CompletionListHelper::deserialize(deserializer)?;
        let defaults = helper.item_defaults.as_ref();
        let items = helper
            .items
            .into_iter()
            .map(|item| {
                let mut completion_item = item.item;
                if let Some(defaults) = defaults {
                    apply_completion_list_defaults(
                        &mut completion_item,
                        item.text_edit_text,
                        defaults,
                    );
        ...
        Ok(Self {
            is_incomplete: helper.is_incomplete,
            items,
        })
    }
}

fn apply_completion_list_defaults(
    item: &mut CompletionItem,
    text_edit_text: Option<String>,
    defaults: &CompletionListItemDefaults,
) {
    if item.text_edit.is_none() {
        if let Some(edit_range) = defaults.edit_range.as_ref() {
            ...
            item.text_edit = Some(match edit_range {
                CompletionListItemDefaultsEditRange::Range(range) => {
                    CompletionTextEdit::Edit(TextEdit::new(*range, new_text))
                }
...
mod tests {
    ...
    #[test]
    fn completion_list_item_defaults_apply_to_items() {
        let list: CompletionList = serde_json::from_str(
            r#"{
                "isIncomplete": false,
                "itemDefaults": {
                    "editRange": {
        ...
        .expect("completion list with item defaults");
        ...
        assert_eq!(
            item.data.as_ref().and_then(|data| data.get("resolve")),
