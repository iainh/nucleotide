...
impl Document {
    ...
    /// Create a new document from an arbitrary reader for a known path.
    ///
    /// This is useful for frontends that load file contents through a virtual
    /// filesystem or remote workspace backend. The caller provides the file
    /// metadata so Helix does not have to probe the local filesystem for paths
    /// that may not exist on the host.
    pub fn open_from_reader<R: std::io::Read + ?Sized>(
        path: &Path,
        reader: &mut R,
        encoding: Option<&'static Encoding>,
        detect_language: bool,
        ...
        let (rope, encoding, has_bom) = from_reader(reader, encoding)?;
        let loader = syn_loader.load();
        let mut doc = Self::from(rope, Some((encoding, has_bom)), config, syn_loader);
    ...
    /// Returns the document's encoding together with whether a BOM should be
    /// emitted when writing.
    pub fn encoding_with_bom_info(&self) -> (&'static Encoding, bool) {
        (self.encoding, self.has_bom)
    }

    /// Returns the file modification time observed by the last successful
    /// open, reload, or save.
    ...
    pub fn set_path(&mut self, path: Option<&Path>) {
        self.set_path_with_metadata(path, None, None);
    }

    /// Sets the document path and externally supplied file metadata without
    /// sending events to observers.
    ///
    /// Callers that load files through non-local backends should pass metadata
    /// from that backend to avoid local filesystem probes for virtual paths.
    pub fn set_path_with_metadata(
        &mut self,
        path: Option<&Path>,
        ...
        if let Some(last_saved_time) = last_saved_time {
