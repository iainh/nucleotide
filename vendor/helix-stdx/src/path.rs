...
pub fn canonicalize(path: impl AsRef<Path>) -> PathBuf {
    ...
    if is_remote_virtual_path(path) {
...
fn is_remote_virtual_path(path: &Path) -> bool {
    let value = path.to_string_lossy();
    if has_prefix_ignore_ascii_case(&value, "ssh://") {
        return true;
    }

    let normalized = value.replace('\\', "/");
    has_prefix_ignore_ascii_case(&normalized, "//wsl.localhost/")
        || has_prefix_ignore_ascii_case(&normalized, "//wsl$/")
}
...
mod tests {
    ...
    #[test]
    fn canonicalize_preserves_ssh_remote_virtual_paths() {
        let path = Path::new("SSH://me@example.com:2222/home/me/project/../src/main.rs");

        assert_eq!(path::canonicalize(path), path);
    }

    #[test]
    fn canonicalize_preserves_wsl_unc_remote_virtual_paths() {
        let backslash_path = Path::new(r"\\WSL.LocalHost\Ubuntu-24.04\home\me\project");
        let slash_path = Path::new("//wsl$/Debian/var/www");

        assert_eq!(path::canonicalize(backslash_path), backslash_path);
        assert_eq!(path::canonicalize(slash_path), slash_path);
    }
