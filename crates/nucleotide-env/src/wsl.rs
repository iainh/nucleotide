...
use nucleotide_remote::{
    ...
    FileReadonlyResponse, FileRenameResponse, FileSearchResponse, FileWriteResponse,
    FormatResponse, GlobalSearchResponse, HelloResponse, PROTOCOL_VERSION,
...
pub fn build_wsl_remote_file_content_command(path: &Path) -> Option<Command> {
    let (workspace, file_name) = wsl_file_parent_workspace(path)?;
    Some(build_wsl_shell_command(
        &workspace,
        "/bin/sh",
        &wsl_remote_helper_file_content_script(&file_name),
    ))
}

pub fn build_wsl_remote_file_write_command(
    path: &Path,
    ...
    Some(build_wsl_shell_command(
        &workspace,
        "/bin/sh",
        &wsl_remote_helper_file_write_script(
            &file_name,
            create_parent_dirs,
...
pub fn build_wsl_remote_set_readonly_command(path: &Path, readonly: bool) -> Option<Command> {
    let (workspace, file_name) = wsl_file_parent_workspace(path)?;
    Some(build_wsl_shell_command(
        &workspace,
        "/bin/sh",
        &wsl_remote_helper_set_readonly_script(&file_name, readonly),
    ))
}

pub fn build_wsl_remote_format_command(
    path: &Path,
    command: &str,
    args: &[String],
) -> Option<Command> {
    let (workspace, _) = wsl_file_parent_workspace(path)?;
    Some(build_wsl_shell_command(
        &workspace,
        "/bin/sh",
        &wsl_remote_helper_format_script(command, args),
    ))
}
...
pub fn build_wsl_remote_delete_path_command(path: &Path) -> Option<Command> {
    let (workspace, name) = wsl_file_parent_workspace(path)?;
    Some(build_wsl_shell_command(
        &workspace,
        "/bin/sh",
        &wsl_remote_helper_delete_path_script(&name),
    ))
}

pub fn build_wsl_remote_duplicate_path_command(path: &Path, target_name: &str) -> Option<Command> {
    let (workspace, old_name) = wsl_file_parent_workspace(path)?;
    Some(build_wsl_shell_command(
        &workspace,
        "/bin/sh",
        &wsl_remote_helper_duplicate_path_script(&old_name, target_name),
    ))
}

pub fn build_wsl_remote_move_path_command(path: &Path, target_path: &Path) -> Option<Command> {
    let (workspace, old_name) = wsl_file_parent_workspace(path)?;
    let target_workspace = WslWorkspace::from_unc_path(target_path)?;
    if target_workspace.distro() != workspace.distro() {
        return None;
    }
    Some(build_wsl_shell_command(
        &workspace,
        "/bin/sh",
        &wsl_remote_helper_move_path_script(&old_name, target_workspace.linux_path()),
    ))
}
...
pub fn wsl_remote_helper_delete_path_script(name: &str) -> String {
    let name = quote_posix_single(name);
    let helper_command = wsl_remote_helper_command_script("delete");
    format!(
        r#"NUCLEOTIDE_REMOTE_DELETE_NAME={name}
export NUCLEOTIDE_REMOTE_DELETE_NAME
{helper_command}"#
    )
}

pub fn wsl_remote_helper_duplicate_path_script(old_name: &str, target_name: &str) -> String {
    let old_name = quote_posix_single(old_name);
    let target_name = quote_posix_single(target_name);
    let helper_command = wsl_remote_helper_command_script("duplicate");
    format!(
        r#"NUCLEOTIDE_REMOTE_DUPLICATE_OLD_NAME={old_name}
NUCLEOTIDE_REMOTE_DUPLICATE_TARGET_NAME={target_name}
export NUCLEOTIDE_REMOTE_DUPLICATE_OLD_NAME NUCLEOTIDE_REMOTE_DUPLICATE_TARGET_NAME
{helper_command}"#
    )
}

pub fn wsl_remote_helper_move_path_script(old_name: &str, target_path: &str) -> String {
    let old_name = quote_posix_single(old_name);
    let target_path = quote_posix_single(target_path);
    let helper_command = wsl_remote_helper_command_script("move");
    format!(
        r#"NUCLEOTIDE_REMOTE_MOVE_OLD_NAME={old_name}
NUCLEOTIDE_REMOTE_MOVE_TARGET_PATH={target_path}
export NUCLEOTIDE_REMOTE_MOVE_OLD_NAME NUCLEOTIDE_REMOTE_MOVE_TARGET_PATH
{helper_command}"#
    )
}
...
pub fn wsl_remote_helper_file_content_script(path: &str) -> String {
    let path = quote_posix_single(path);
    let helper_command = wsl_remote_helper_command_script("read-full");
    format!(
        r#"NUCLEOTIDE_REMOTE_READ_PATH={path}
export NUCLEOTIDE_REMOTE_READ_PATH
{helper_command}"#
    )
}

pub fn wsl_remote_helper_file_write_script(
    path: &str,
    create_parent_dirs: bool,
    expected_modified_unix_millis: Option<i64>,
) -> String {
    let path = quote_posix_single(path);
    let create_parent_dirs = env_bool_value(create_parent_dirs);
    let expected_modified_unix_millis = expected_modified_unix_millis
        .map(|value| value.to_string())
        .unwrap_or_default();
    let helper_command = wsl_remote_helper_command_script("write");
    format!(
        r#"NUCLEOTIDE_REMOTE_WRITE_PATH={path}
NUCLEOTIDE_REMOTE_WRITE_CREATE_PARENT_DIRS={create_parent_dirs}
NUCLEOTIDE_REMOTE_WRITE_EXPECTED_MTIME_UNIX_MILLIS={expected_modified_unix_millis}
export NUCLEOTIDE_REMOTE_WRITE_PATH NUCLEOTIDE_REMOTE_WRITE_CREATE_PARENT_DIRS NUCLEOTIDE_REMOTE_WRITE_EXPECTED_MTIME_UNIX_MILLIS
{helper_command}"#
    )
}

pub fn wsl_remote_helper_set_readonly_script(path: &str, readonly: bool) -> String {
    let path = quote_posix_single(path);
    let readonly = env_bool_value(readonly);
    let helper_command = wsl_remote_helper_command_script("set-readonly");
    format!(
        r#"NUCLEOTIDE_REMOTE_READONLY_PATH={path}
NUCLEOTIDE_REMOTE_READONLY_VALUE={readonly}
export NUCLEOTIDE_REMOTE_READONLY_PATH NUCLEOTIDE_REMOTE_READONLY_VALUE
{helper_command}"#
    )
}

pub fn wsl_remote_helper_format_script(command: &str, args: &[String]) -> String {
    let command = quote_posix_single(command);
    let args_json = serde_json::to_string(args).unwrap_or_else(|_| "[]".to_string());
    let args_json = quote_posix_single(&args_json);
    let helper_command = wsl_remote_helper_command_script("format");
    format!(
        r#"NUCLEOTIDE_REMOTE_FORMAT_COMMAND={command}
NUCLEOTIDE_REMOTE_FORMAT_ARGS_JSON={args_json}
export NUCLEOTIDE_REMOTE_FORMAT_COMMAND NUCLEOTIDE_REMOTE_FORMAT_ARGS_JSON
{helper_command}"#
    )
}
...
pub fn load_wsl_remote_file_content_blocking(
    path: &Path,
    timeout_duration: Duration,
...
pub fn write_wsl_remote_file_blocking(
    path: &Path,
    content: &[u8],
    create_parent_dirs: bool,
    expected_modified_unix_millis: Option<i64>,
...
pub fn set_wsl_remote_readonly_blocking(
    path: &Path,
    readonly: bool,
    timeout_duration: Duration,
...
pub fn format_wsl_remote_file_blocking(
    path: &Path,
    command: &str,
    args: &[String],
    content: &[u8],
...
pub fn delete_wsl_remote_path_blocking(
    path: &Path,
    timeout_duration: Duration,
...
pub fn duplicate_wsl_remote_path_blocking(
    path: &Path,
    target_name: &str,
    timeout_duration: Duration,
...
pub fn move_wsl_remote_path_blocking(
    path: &Path,
    target_path: &Path,
    timeout_duration: Duration,
...
fn run_blocking_wsl_remote_command_inner(
    ...
    loop {
        if let Some(status) = child.try_wait()? {
            let stdin_result = stdin_writer.map(|stdin_writer| {
                stdin_writer
                    .join()
...
fn parse_remote_file_content_output(
    output: &[u8],
) -> Result<FileContentResponse, WslRemoteHelperError> {
    let response: FileContentResponse = serde_json::from_slice(output)?;
    if response.protocol_version != PROTOCOL_VERSION {
        return Err(WslRemoteHelperError::ProtocolMismatch {
            expected: PROTOCOL_VERSION,
            actual: response.protocol_version,
...
fn parse_remote_file_write_output(
    output: &[u8],
) -> Result<FileWriteResponse, WslRemoteHelperError> {
    let response: FileWriteResponse = serde_json::from_slice(output)?;
    if response.protocol_version != PROTOCOL_VERSION {
        return Err(WslRemoteHelperError::ProtocolMismatch {
            expected: PROTOCOL_VERSION,
            actual: response.protocol_version,
...
fn parse_remote_file_readonly_output(
    output: &[u8],
) -> Result<FileReadonlyResponse, WslRemoteHelperError> {
    let response: FileReadonlyResponse = serde_json::from_slice(output)?;
    if response.protocol_version != PROTOCOL_VERSION {
        return Err(WslRemoteHelperError::ProtocolMismatch {
            expected: PROTOCOL_VERSION,
            actual: response.protocol_version,
...
fn parse_remote_format_output(output: &[u8]) -> Result<FormatResponse, WslRemoteHelperError> {
    let response: FormatResponse = serde_json::from_slice(output)?;
    if response.protocol_version != PROTOCOL_VERSION {
        return Err(WslRemoteHelperError::ProtocolMismatch {
            expected: PROTOCOL_VERSION,
            actual: response.protocol_version,
...
fn parse_remote_file_delete_output(
    output: &[u8],
) -> Result<FileDeleteResponse, WslRemoteHelperError> {
    let response: FileDeleteResponse = serde_json::from_slice(output)?;
    if response.protocol_version != PROTOCOL_VERSION {
        return Err(WslRemoteHelperError::ProtocolMismatch {
            expected: PROTOCOL_VERSION,
            actual: response.protocol_version,
...
fn parse_remote_file_duplicate_output(
    output: &[u8],
) -> Result<FileDuplicateResponse, WslRemoteHelperError> {
    let response: FileDuplicateResponse = serde_json::from_slice(output)?;
    if response.protocol_version != PROTOCOL_VERSION {
        return Err(WslRemoteHelperError::ProtocolMismatch {
            expected: PROTOCOL_VERSION,
            actual: response.protocol_version,
...
fn parse_remote_file_move_output(output: &[u8]) -> Result<FileMoveResponse, WslRemoteHelperError> {
    let response: FileMoveResponse = serde_json::from_slice(output)?;
    if response.protocol_version != PROTOCOL_VERSION {
        return Err(WslRemoteHelperError::ProtocolMismatch {
            expected: PROTOCOL_VERSION,
            actual: response.protocol_version,
...
mod tests {
    ...
    fn remote_helper_hello_script_prefers_cached_helper_before_path() {
        ...
        assert!(script.contains(".cache/nucleotide/remote-helper/17/nucleotide-remote"));
    ...
    #[test]
    fn remote_helper_delete_path_script_passes_delete_environment() {
        let script = wsl_remote_helper_delete_path_script("delete me.rs");

        assert!(script.contains("NUCLEOTIDE_REMOTE_DELETE_NAME='delete me.rs'"));
        assert!(script.contains(".cache/nucleotide/remote-helper/17/nucleotide-remote"));
        assert!(script.contains(r#"exec "$helper" delete"#));
        assert!(script.contains("exec nucleotide-remote delete"));
    }

    #[test]
    fn remote_helper_duplicate_path_script_passes_duplicate_environment() {
        let script = wsl_remote_helper_duplicate_path_script("old file.rs", "new file.rs");

        assert!(script.contains("NUCLEOTIDE_REMOTE_DUPLICATE_OLD_NAME='old file.rs'"));
        assert!(script.contains("NUCLEOTIDE_REMOTE_DUPLICATE_TARGET_NAME='new file.rs'"));
        assert!(script.contains(".cache/nucleotide/remote-helper/17/nucleotide-remote"));
        assert!(script.contains(r#"exec "$helper" duplicate"#));
        assert!(script.contains("exec nucleotide-remote duplicate"));
    }

    #[test]
    fn remote_helper_move_path_script_passes_move_environment() {
        let script =
            wsl_remote_helper_move_path_script("old file.rs", "/home/iain/repo/new file.rs");

        assert!(script.contains("NUCLEOTIDE_REMOTE_MOVE_OLD_NAME='old file.rs'"));
        assert!(
            script.contains("NUCLEOTIDE_REMOTE_MOVE_TARGET_PATH='/home/iain/repo/new file.rs'")
        );
        assert!(script.contains(".cache/nucleotide/remote-helper/17/nucleotide-remote"));
        assert!(script.contains(r#"exec "$helper" move"#));
        assert!(script.contains("exec nucleotide-remote move"));
    }
    ...
    #[test]
    fn remote_helper_file_content_script_passes_read_environment() {
        let script = wsl_remote_helper_file_content_script("it isn't easy.rs");

        assert!(script.contains("NUCLEOTIDE_REMOTE_READ_PATH='it isn'\"'\"'t easy.rs'"));
        assert!(script.contains(".cache/nucleotide/remote-helper/17/nucleotide-remote"));
        assert!(script.contains(r#"exec "$helper" read-full"#));
        assert!(script.contains("exec nucleotide-remote read-full"));
    }

    #[test]
    fn remote_helper_file_write_script_passes_write_environment() {
        let script = wsl_remote_helper_file_write_script("it isn't easy.rs", true, Some(1234));

        assert!(script.contains("NUCLEOTIDE_REMOTE_WRITE_PATH='it isn'\"'\"'t easy.rs'"));
        assert!(script.contains("NUCLEOTIDE_REMOTE_WRITE_CREATE_PARENT_DIRS=1"));
        assert!(script.contains("NUCLEOTIDE_REMOTE_WRITE_EXPECTED_MTIME_UNIX_MILLIS=1234"));
        assert!(script.contains(".cache/nucleotide/remote-helper/17/nucleotide-remote"));
        assert!(script.contains(r#"exec "$helper" write"#));
        assert!(script.contains("exec nucleotide-remote write"));
    }

    #[test]
    fn remote_helper_set_readonly_script_passes_readonly_environment() {
        let script = wsl_remote_helper_set_readonly_script("it isn't easy.rs", true);

        assert!(script.contains("NUCLEOTIDE_REMOTE_READONLY_PATH='it isn'\"'\"'t easy.rs'"));
        assert!(script.contains("NUCLEOTIDE_REMOTE_READONLY_VALUE=1"));
        assert!(script.contains(".cache/nucleotide/remote-helper/17/nucleotide-remote"));
        assert!(script.contains(r#"exec "$helper" set-readonly"#));
        assert!(script.contains("exec nucleotide-remote set-readonly"));
    }

    #[test]
    fn remote_helper_format_script_passes_formatter_environment() {
        let args = vec!["--stdin-filepath".to_string(), "src/main.rs".to_string()];
        let script = wsl_remote_helper_format_script("prettier", &args);

        assert!(script.contains("NUCLEOTIDE_REMOTE_FORMAT_COMMAND='prettier'"));
        assert!(script.contains(
            "NUCLEOTIDE_REMOTE_FORMAT_ARGS_JSON='[\"--stdin-filepath\",\"src/main.rs\"]'"
        ));
        assert!(script.contains(".cache/nucleotide/remote-helper/17/nucleotide-remote"));
        assert!(script.contains(r#"exec "$helper" format"#));
        assert!(script.contains("exec nucleotide-remote format"));
    }
    ...
    #[test]
    fn builds_wsl_remote_delete_path_command() {
        let path = Path::new(r"\\wsl.localhost\Ubuntu\home\iain\repo\src\delete.rs");
        let command = build_wsl_remote_delete_path_command(path).expect("delete path command");
        let debug = format!("{command:?}");
        ...
        assert!(debug.contains("--cd"));
        assert!(debug.contains("/home/iain/repo/src"));
        assert!(debug.contains("NUCLEOTIDE_REMOTE_DELETE_NAME"));
        assert!(debug.contains("delete.rs"));
        assert!(debug.contains("delete"));
    }

    #[test]
    fn builds_wsl_remote_duplicate_path_command() {
        let path = Path::new(r"\\wsl.localhost\Ubuntu\home\iain\repo\src\old.rs");
        let command = build_wsl_remote_duplicate_path_command(path, "old copy.rs")
            .expect("duplicate path command");
        let debug = format!("{command:?}");
        ...
        assert!(debug.contains("--cd"));
        assert!(debug.contains("/home/iain/repo/src"));
        assert!(debug.contains("NUCLEOTIDE_REMOTE_DUPLICATE_OLD_NAME"));
        assert!(debug.contains("old.rs"));
        assert!(debug.contains("NUCLEOTIDE_REMOTE_DUPLICATE_TARGET_NAME"));
        assert!(debug.contains("old copy.rs"));
        assert!(debug.contains("duplicate"));
    }

    #[test]
    fn builds_wsl_remote_move_path_command() {
        let path = Path::new(r"\\wsl.localhost\Ubuntu\home\iain\repo\src\old.rs");
        let target = Path::new(r"\\wsl.localhost\Ubuntu\home\iain\repo\crates\old.rs");
        let command = build_wsl_remote_move_path_command(path, target).expect("move path command");
        let debug = format!("{command:?}");
        ...
        assert!(debug.contains("--cd"));
        assert!(debug.contains("/home/iain/repo/src"));
        assert!(debug.contains("NUCLEOTIDE_REMOTE_MOVE_OLD_NAME"));
        assert!(debug.contains("old.rs"));
        assert!(debug.contains("NUCLEOTIDE_REMOTE_MOVE_TARGET_PATH"));
        assert!(debug.contains("/home/iain/repo/crates/old.rs"));
        assert!(debug.contains("move"));
    }

    #[test]
    fn rejects_wsl_remote_move_path_command_across_distros() {
        let path = Path::new(r"\\wsl.localhost\Ubuntu\home\iain\repo\src\old.rs");
        let target = Path::new(r"\\wsl.localhost\Debian\home\iain\repo\crates\old.rs");

        assert!(build_wsl_remote_move_path_command(path, target).is_none());
    }
    ...
    #[test]
    fn builds_wsl_remote_file_content_command_from_file_parent() {
        let command = build_wsl_remote_file_content_command(Path::new(
            r"\\wsl.localhost\Ubuntu\home\iain\repo\src\main.rs",
        ))
        .expect("expected WSL full read command");
        let debug = format!("{command:?}");
        ...
        assert!(debug.contains("main.rs"));
        assert!(debug.contains("nucleotide-remote"));
        assert!(debug.contains("read-full"));
    }

    #[test]
    fn builds_wsl_remote_file_write_command_from_file_parent() {
        let command = build_wsl_remote_file_write_command(
            Path::new(r"\\wsl.localhost\Ubuntu\home\iain\repo\src\main.rs"),
            true,
            Some(1234),
        )
        .expect("expected WSL write command");
        let debug = format!("{command:?}");
        ...
        assert!(debug.contains("--cd"));
        assert!(debug.contains("/home/iain/repo/src"));
        assert!(debug.contains("NUCLEOTIDE_REMOTE_WRITE_PATH"));
        assert!(debug.contains("main.rs"));
        assert!(debug.contains("NUCLEOTIDE_REMOTE_WRITE_CREATE_PARENT_DIRS=1"));
        assert!(debug.contains("NUCLEOTIDE_REMOTE_WRITE_EXPECTED_MTIME_UNIX_MILLIS=1234"));
        assert!(debug.contains("nucleotide-remote"));
    ...
    fn remote_helper_install_script_writes_versioned_cache_path() {
        ...
        assert!(
            script
                .contains(r#"helper="$HOME/.cache/nucleotide/remote-helper/17/nucleotide-remote""#)
        );
    ...
    fn parses_remote_hello_response() {
        let output = br#"{"protocol_version":17,"helper_version":"0.1.0","os":"linux","arch":"x86_64","current_dir":"/home/iain/repo"}
"#;
    ...
    fn parses_remote_environment_response() {
        let output = br#"{"protocol_version":17,"current_dir":"/home/iain/repo","variables":{"PATH":"/usr/bin","SHELL":"/bin/bash"}}
"#;
    ...
    fn parses_remote_metadata_response() {
        let output = br#"{"protocol_version":17,"helper_version":"0.1.0","os":"linux","arch":"x86_64","current_dir":"/home/iain/repo","home_dir":"/home/iain","path_separator":"/"}
"#;
    ...
    fn parses_remote_workspace_root_response() {
        let output = br#"{"protocol_version":17,"current_dir":"/home/iain/repo/src","workspace_root":"/home/iain/repo","workspace_marker":".git","project_root":"/home/iain/repo","project_marker":"Cargo.toml"}
"#;
    ...
    fn parses_remote_directory_listing_response() {
        let output = br#"{"protocol_version":17,"current_dir":"/home/iain/repo","entries":[{"name":"src","kind":"directory","size":4096,"modified_unix_millis":1000,"symlink_target":null,"target_exists":null}]}
"#;
    ...
    fn parses_remote_file_search_response() {
        let output = br#"{"protocol_version":17,"current_dir":"/home/iain/repo","files":[{"relative_path":"src/main.rs"}],"truncated":false}
"#;
    ...
    fn parses_remote_global_search_response() {
        let output = br#"{"protocol_version":17,"current_dir":"/home/iain/repo","matches":[{"relative_path":"src/main.rs","line":7,"line_text":"needle"}],"truncated":false}
"#;
    ...
    fn parses_remote_file_read_response() {
        let output = br#"{"protocol_version":17,"current_dir":"/home/iain/repo/src","path":"main.rs","content":"fn main() {}\n","binary":false,"size":13,"truncated":false}
"#;
    ...
    #[test]
    fn parses_remote_file_content_response() {
        let output = br#"{"protocol_version":17,"current_dir":"/home/iain/repo/src","path":"main.rs","content_base64":"AP9oaQ==","size":5,"modified_unix_millis":1700000000000,"readonly":false}
"#;

        let response = parse_remote_file_content_output(output).unwrap();

        assert_eq!(response.protocol_version, PROTOCOL_VERSION);
        assert_eq!(response.current_dir, Path::new("/home/iain/repo/src"));
        assert_eq!(response.path, Path::new("main.rs"));
        assert_eq!(response.content_base64, "AP9oaQ==");
        assert_eq!(response.size, 5);
        assert_eq!(response.modified_unix_millis, Some(1_700_000_000_000));
        assert!(!response.readonly);
    }

    #[test]
    fn rejects_remote_file_content_protocol_mismatch() {
        let output = br#"{"protocol_version":999,"current_dir":"/home/iain/repo","path":"main.rs","content_base64":"","size":0,"modified_unix_millis":null,"readonly":false}"#;

        let error = parse_remote_file_content_output(output).unwrap_err();
    ...
    #[test]
    fn parses_remote_file_write_response() {
        let output = br#"{"protocol_version":17,"current_dir":"/home/iain/repo/src","path":"main.rs","size":13,"modified_unix_millis":1700000000000}
"#;

        let response = parse_remote_file_write_output(output).unwrap();
    ...
    #[test]
    fn parses_remote_file_readonly_response() {
        let output = br#"{"protocol_version":17,"current_dir":"/home/iain/repo/src","path":"main.rs","readonly":true}
"#;

        let response = parse_remote_file_readonly_output(output).unwrap();
    ...
    #[test]
    fn parses_remote_format_response() {
        let output =
            br#"{"protocol_version":17,"current_dir":"/home/iain/repo/src","output_base64":"QUJD"}
"#;

        let response = parse_remote_format_output(output).unwrap();

        assert_eq!(response.protocol_version, PROTOCOL_VERSION);
        assert_eq!(response.current_dir, Path::new("/home/iain/repo/src"));
        assert_eq!(response.output_base64, "QUJD");
    }

    #[test]
    fn rejects_remote_file_write_protocol_mismatch() {
        let output = br#"{"protocol_version":999,"current_dir":"/home/iain/repo","path":"main.rs","size":3,"modified_unix_millis":null}"#;

        let error = parse_remote_file_write_output(output).unwrap_err();
    ...
    fn parses_remote_file_create_response() {
        let output = br#"{"protocol_version":17,"current_dir":"/home/iain/repo/src","path":"/home/iain/repo/src/main.rs","kind":"file"}
"#;
    ...
    fn parses_remote_directory_create_response() {
        let output = br#"{"protocol_version":17,"current_dir":"/home/iain/repo/src","path":"/home/iain/repo/src/components","kind":"directory"}
"#;
    ...
    fn parses_remote_file_rename_response() {
        let output = br#"{"protocol_version":17,"current_dir":"/home/iain/repo/src","old_path":"/home/iain/repo/src/old.rs","new_path":"/home/iain/repo/src/new.rs","kind":"file"}
"#;
    ...
    #[test]
    fn parses_remote_file_delete_response() {
        let output = br#"{"protocol_version":17,"current_dir":"/home/iain/repo/src","path":"/home/iain/repo/src/delete.rs","kind":"file"}
"#;

        let response = parse_remote_file_delete_output(output).unwrap();

        assert_eq!(response.protocol_version, PROTOCOL_VERSION);
        assert_eq!(response.current_dir, Path::new("/home/iain/repo/src"));
        assert_eq!(response.path, Path::new("/home/iain/repo/src/delete.rs"));
        assert_eq!(response.kind, nucleotide_remote::RemoteFileKind::File);
    }

    #[test]
    fn rejects_remote_file_delete_protocol_mismatch() {
        let output = br#"{"protocol_version":999,"current_dir":"/home/iain/repo/src","path":"/home/iain/repo/src/delete.rs","kind":"file"}"#;

        let error = parse_remote_file_delete_output(output).unwrap_err();
    ...
    #[test]
    fn parses_remote_file_duplicate_response() {
        let output = br#"{"protocol_version":17,"current_dir":"/home/iain/repo/src","old_path":"/home/iain/repo/src/old.rs","new_path":"/home/iain/repo/src/old copy.rs","kind":"file"}
"#;

        let response = parse_remote_file_duplicate_output(output).unwrap();
        ...
        assert_eq!(
            response.new_path,
            Path::new("/home/iain/repo/src/old copy.rs")
        );
        assert_eq!(response.kind, nucleotide_remote::RemoteFileKind::File);
    }

    #[test]
    fn rejects_remote_file_duplicate_protocol_mismatch() {
        let output = br#"{"protocol_version":999,"current_dir":"/home/iain/repo/src","old_path":"/home/iain/repo/src/old.rs","new_path":"/home/iain/repo/src/old copy.rs","kind":"file"}"#;

        let error = parse_remote_file_duplicate_output(output).unwrap_err();
    ...
    #[test]
    fn parses_remote_file_move_response() {
        let output = br#"{"protocol_version":17,"current_dir":"/home/iain/repo/src","old_path":"/home/iain/repo/src/old.rs","new_path":"/home/iain/repo/crates/old.rs","kind":"file"}
"#;

        let response = parse_remote_file_move_output(output).unwrap();
        ...
        assert_eq!(
            response.new_path,
            Path::new("/home/iain/repo/crates/old.rs")
        );
        assert_eq!(response.kind, nucleotide_remote::RemoteFileKind::File);
    }

    #[test]
    fn rejects_remote_file_move_protocol_mismatch() {
        let output = br#"{"protocol_version":999,"current_dir":"/home/iain/repo/src","old_path":"/home/iain/repo/src/old.rs","new_path":"/home/iain/repo/crates/old.rs","kind":"file"}"#;

        let error = parse_remote_file_move_output(output).unwrap_err();
    ...
    fn parses_remote_workspace_symbol_files_response() {
        let output = br#"{"protocol_version":17,"current_dir":"/home/iain/repo","files":[{"relative_path":"src/main.rs","content":"fn main() {}\n","size":13}],"truncated":false}
"#;
