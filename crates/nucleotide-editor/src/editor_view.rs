...
use gpui::{Context, IntoElement, ParentElement, Pixels, Render, Styled, Window, div, px};
...
pub struct EditorView<S: EditorState> {
    ...
    /// Scroll state
    scroll_state: crate::scroll_state::ScrollState,
...
impl<S: EditorState + 'static> EditorView<S> {
    ...
    pub fn new(editor_state: Arc<RwLock<S>>) -> Self {
        Self {
            ...
            current_doc: None,
            current_view: None,
            scroll_state: crate::scroll_state::ScrollState::new(),
            line_height: px(20.0),
        }
    }
