...
impl ThemeProvider {
    ...
    pub fn with_themes(themes: HashMap<SharedString, Theme>, default_theme_name: &str) -> Self {
        let current_theme = themes.get(default_theme_name)
            .cloned()
    ...
    pub fn remove_theme(&mut self, name: &str) -> bool {
        if self.available_themes.len() <= 1 {
            nucleotide_logging::warn!(
                theme_name = name,
                "Cannot remove last available theme"
            );
            return false;
        }
    ...
    pub fn override_color(&mut self, color_key: impl Into<String>, color: gpui::Hsla) {
        self.theme_overrides.color_overrides.insert(color_key.into(), color);
    ...
    pub fn override_size(&mut self, size_key: impl Into<String>, size: gpui::Pixels) {
        self.theme_overrides.size_overrides.insert(size_key.into(), size);
        self.current_theme = self.apply_overrides(self.current_theme.clone());
    }
    ...
    pub fn derive_theme(&self, _name: impl Into<SharedString>, modifier: impl FnOnce(&mut Theme)) -> Theme {
...
impl ThemeConfigurations {
    ...
    pub fn high_contrast() -> ThemeProvider {
        ...
        high_contrast_light.tokens.colors.text_primary = gpui::Hsla { h: 0.0, s: 0.0, l: 0.0, a: 1.0 };
        high_contrast_light.tokens.colors.background = gpui::Hsla { h: 0.0, s: 0.0, l: 1.0, a: 1.0 };
        
        let mut high_contrast_dark = Theme::dark();
        high_contrast_dark.tokens.colors.text_primary = gpui::Hsla { h: 0.0, s: 0.0, l: 1.0, a: 1.0 };
        high_contrast_dark.tokens.colors.background = gpui::Hsla { h: 0.0, s: 0.0, l: 0.0, a: 1.0 };
    ...
    pub fn with_brand_colors(primary_color: gpui::Hsla, secondary_color: gpui::Hsla) -> ThemeProvider {
        let mut provider = Self::light_dark();
...
mod tests {
    ...
    fn test_theme_overrides() {
        let mut provider = ThemeProvider::new(Theme::light());
        let custom_color = gpui::Hsla { h: 200.0, s: 0.8, l: 0.5, a: 1.0 };
    ...
    fn test_theme_configurations() {
        ...
        let brand_color = gpui::Hsla { h: 220.0, s: 0.9, l: 0.6, a: 1.0 };
        let secondary_color = gpui::Hsla { h: 180.0, s: 0.7, l: 0.5, a: 1.0 };
        let brand_provider = ThemeConfigurations::with_brand_colors(brand_color, secondary_color);
        
        assert_eq!(brand_provider.current_theme().tokens.colors.primary, brand_color);
        // Note: "secondary" maps to text_secondary since there's no dedicated secondary color field
        assert_eq!(brand_provider.current_theme().tokens.colors.text_secondary, secondary_color);
    }
    ...
    fn test_derive_theme() {
        ...
        let derived = provider.derive_theme("custom", |theme| {
            theme.tokens.colors.primary = gpui::Hsla { h: 300.0, s: 0.8, l: 0.6, a: 1.0 };
        });
        
        // Original theme should be unchanged
        assert_ne!(provider.current_theme().tokens.colors.primary, derived.tokens.colors.primary);
        
        // Derived theme should have the modification
        assert_eq!(
            derived.tokens.colors.primary,
            gpui::Hsla { h: 300.0, s: 0.8, l: 0.6, a: 1.0 }
        );
    }
