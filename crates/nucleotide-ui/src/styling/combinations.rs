...
use super::{ComputedStyle, StyleState, BoxShadow, Transition};
use gpui::{Hsla, px};
...
impl StyleComposer {
    ...
    where
        F: Fn() -> bool + 'static,
    {
        self.conditional_styles.push(ConditionalStyle::new(condition, style));
        self
    }
...
impl StylePresets {
    ...
    pub fn button_set(base: ComputedStyle, focus_color: Hsla, is_dark_theme: bool) -> HashMap<StyleState, ComputedStyle> {
        let mut styles = HashMap::new();
        
        styles.insert(StyleState::Default, base.clone());
        styles.insert(StyleState::Hover, StyleUtils::create_hover_style(&base, is_dark_theme));
        styles.insert(StyleState::Active, StyleUtils::create_active_style(&base, is_dark_theme));
        styles.insert(StyleState::Disabled, StyleUtils::create_disabled_style(&base));
        styles.insert(StyleState::Focused, StyleUtils::create_focus_style(&base, focus_color));
        
        styles
    }
    
    /// Create a card style set with elevation
    pub fn card_set(base: ComputedStyle, elevation_levels: &[u8]) -> HashMap<String, ComputedStyle> {
        let mut styles = HashMap::new();
