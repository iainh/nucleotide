...
pub trait CustomStorage: std::fmt::Debug + Send + Sync {
    /// Save theme choice
    fn save_theme(&self, key: &str, theme_name: &str, theme_data: &Theme) -> Result<(), StorageError>;
...
impl RuntimeThemeSwitcher {
    ...
    pub fn switch_theme_immediate(
        ...
        let previous_state = {
            if let Ok(mut state) = self.current_state.write() {
                ...
                return Err(SwitchingError::LockError("Failed to acquire state lock".into()));
            }
        };
        ...
        let (_completion_time, duration) = {
            if let Ok(mut state) = self.current_state.write() {
                ...
                return Err(SwitchingError::LockError("Failed to acquire state lock for switch".into()));
            }
        };
        ...
        let switch_result = {
            ...
            if self.config.enable_history {
                self.add_to_history(new_theme.clone(), theme_name.clone(), ThemeOperation::UserSwitch, None);
            }
    ...
    pub fn switch_to_previous(&mut self) -> Result<SwitchingResult, SwitchingError> {
        let (previous_theme, previous_name) = {
            if let Ok(state) = self.current_state.read() {
                (state.previous_theme.clone(), state.previous_theme_name.clone())
            } else {
    ...
    pub fn load_persisted_theme(&mut self) -> Result<Option<(SharedString, Theme)>, SwitchingError> {
        match &self.persistence.backend {
            StorageBackend::FileSystem { config_dir } => {
                let theme_file = config_dir.join(&self.config.storage_key).with_extension("json");
            ...
            StorageBackend::Custom(storage) => {
                match storage.load_theme(&self.config.storage_key) {
    ...
    pub fn history_previous(&mut self) -> Result<Option<SwitchingResult>, SwitchingError> {
        let history_entry = {
            if let Ok(mut history) = self.theme_history.write() {
                ...
                return Err(SwitchingError::LockError("Failed to acquire history lock".into()));
            }
        };
    ...
    pub fn clear_history(&mut self) -> Result<(), SwitchingError> {
        if let Ok(mut history) = self.theme_history.write() {
            ...
            Err(SwitchingError::LockError("Failed to acquire history lock".into()))
        }
    }
    ...
    pub fn get_performance_stats(&self) -> SwitchingPerformanceStats {
        SwitchingPerformanceStats {
            ...
            success_rate: if self.performance_monitor.total_switches > 0 {
                ((self.performance_monitor.total_switches - self.performance_monitor.failed_switches) as f32
                    / self.performance_monitor.total_switches as f32) * 100.0
            } else {
                0.0
            },
            recent_switches: self.performance_monitor.switch_metrics
                .iter()
                .rev()
                .take(10)
                .cloned()
    ...
    fn persist_theme_choice(&mut self, theme_name: &str, theme: &Theme) -> Result<(), SwitchingError> {
        match &self.persistence.backend {
            StorageBackend::FileSystem { config_dir } => {
            ...
            StorageBackend::Custom(storage) => {
                storage.save_theme(&self.config.storage_key, theme_name, theme)
                    .map_err(|e| SwitchingError::PersistenceError(e.to_string()))
            }
        }
    }
    ...
    fn record_switch_metric(&mut self, metric: SwitchMetric) {
        ...
        let total_time: Duration = self.performance_monitor.switch_metrics
            .iter()
            .map(|m| m.duration)
            .sum();
        
        self.performance_monitor.average_switch_time = total_time / self.performance_monitor.switch_metrics.len() as u32;
...
mod tests {
    ...
    fn test_immediate_theme_switch() {
        ...
        let result = switcher.switch_theme_immediate(light_theme.clone(), Some("light".into())).unwrap();
    ...
    fn test_previous_theme_switch() {
        ...
        switcher.switch_theme_immediate(light_theme, Some("light".into())).unwrap();
    ...
    fn test_theme_history() {
        ...
        assert_eq!(history.entries.last().unwrap().theme_name, Some("light".into()));
    }
    ...
    fn test_performance_monitoring() {
        ...
        switcher.switch_theme_immediate(light_theme.clone(), Some("light".into())).unwrap();
    ...
    fn test_history_navigation() {
        ...
        // Create some history
        switcher.switch_theme_immediate(light_theme.clone(), Some("light".into())).unwrap();
        switcher.switch_theme_immediate(Theme::dark(), Some("dark".into())).unwrap();
