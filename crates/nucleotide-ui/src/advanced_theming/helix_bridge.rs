...
impl Default for ColorMappings {
    fn default() -> Self {
        ...
        ui_mappings.insert("ui.background.separator".to_string(), "border_default".to_string());
        ...
        fallback_colors.insert("primary".to_string(), Hsla { h: 220.0, s: 0.8, l: 0.6, a: 1.0 });
        fallback_colors.insert("secondary".to_string(), Hsla { h: 180.0, s: 0.7, l: 0.5, a: 1.0 });
        fallback_colors.insert("background".to_string(), Hsla { h: 0.0, s: 0.0, l: 0.95, a: 1.0 });
        fallback_colors.insert("text_primary".to_string(), Hsla { h: 0.0, s: 0.0, l: 0.1, a: 1.0 });
...
impl HelixThemeBridge {
    ...
    pub fn load_helix_theme(&mut self, theme_name: &str) -> Result<Theme, HelixBridgeError> {
        ...
        let theme_info = discovery.themes.iter()
        ...
        self.theme_cache.insert(theme_name.to_string(), cached_theme);
    ...
    pub fn convert_nucleotide_to_helix(&self, theme: &Theme, name: &str) -> Result<HelixThemeData, HelixBridgeError> {
        let mut palette = HashMap::new();
        let mut ui = HashMap::new();
        let mut syntax = HashMap::new();
        
        // Convert colors to hex strings
        palette.insert("primary".to_string(), self.hsla_to_hex(theme.tokens.colors.primary));
        palette.insert("secondary".to_string(), self.hsla_to_hex(theme.tokens.colors.text_secondary));
        palette.insert("background".to_string(), self.hsla_to_hex(theme.tokens.colors.background));
        palette.insert("surface".to_string(), self.hsla_to_hex(theme.tokens.colors.surface));
        palette.insert("text_primary".to_string(), self.hsla_to_hex(theme.tokens.colors.text_primary));
        palette.insert("text_secondary".to_string(), self.hsla_to_hex(theme.tokens.colors.text_secondary));
        palette.insert("border_default".to_string(), self.hsla_to_hex(theme.tokens.colors.border_default));
        palette.insert("error".to_string(), self.hsla_to_hex(theme.tokens.colors.error));
        palette.insert("warning".to_string(), self.hsla_to_hex(theme.tokens.colors.warning));
        palette.insert("success".to_string(), self.hsla_to_hex(theme.tokens.colors.success));
    ...
    pub fn export_to_helix_toml(&self, theme: &Theme, name: &str) -> Result<String, HelixBridgeError> {
        ...
        toml_content.push_str(&format!("# Generated on: {:?}\n\n", std::time::SystemTime::now()));
    ...
    fn scan_theme_directory(&self, path: &Path) -> Result<Vec<DiscoveredHelixTheme>, Vec<ThemeDiscoveryError>> {
        let mut themes = Vec::new();
        let mut errors = Vec::new();
            ...
            match entry {
                Ok(entry) => {
                    ...
                    if let Some(extension) = path.extension() {
                        if self.discovery_config.theme_extensions.contains(&extension.to_string_lossy().to_string()) {
    ...
    fn parse_theme_metadata(&self, path: &Path) -> Result<DiscoveredHelixTheme, ThemeDiscoveryError> {
    ...
    fn convert_helix_to_nucleotide(&self, helix_data: &HelixThemeData) -> Result<Theme, HelixBridgeError> {
    ...
    fn parse_helix_color(&self, color_str: &str, palette: &HashMap<String, String>) -> Result<Hsla, HelixBridgeError> {
        ...
        match color_str {
            "black" => Ok(Hsla { h: 0.0, s: 0.0, l: 0.0, a: 1.0 }),
            "white" => Ok(Hsla { h: 0.0, s: 0.0, l: 1.0, a: 1.0 }),
            "red" => Ok(Hsla { h: 0.0, s: 1.0, l: 0.5, a: 1.0 }),
            "green" => Ok(Hsla { h: 120.0, s: 1.0, l: 0.5, a: 1.0 }),
            "blue" => Ok(Hsla { h: 240.0, s: 1.0, l: 0.5, a: 1.0 }),
    ...
    fn hex_to_hsla(&self, hex: &str) -> Result<Hsla, HelixBridgeError> {
        ...
        let r = u8::from_str_radix(&hex[0..2], 16)
            .map_err(|_| HelixBridgeError::InvalidColor(hex.to_string()))? as f32 / 255.0;
...
mod tests {
    ...
    fn test_hsla_to_hex_conversion() {
        ...
        let blue_hsla = Hsla { h: 240.0, s: 1.0, l: 0.5, a: 1.0 };
