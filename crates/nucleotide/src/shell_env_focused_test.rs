...
mod focused_shell_env_tests {
    use crate::shell_env::{ProjectEnvironment, detect_shell_type, shell_command_builder, parse_shell_environment};
    ...
    async fn test_project_environment_cli_priority() {
        ...
        let result = project_env.get_environment_for_directory(Path::new("/any/dir")).await;
        assert!(result.is_ok());
    ...
    async fn test_shell_command_building() {
        ...
        let fish_cmd = shell_command_builder::build_shell_command("/usr/local/bin/fish", Path::new("/test"));  
    ...
    async fn test_environment_capture_command() {
        let result = shell_command_builder::build_environment_capture_command("/bin/bash", Path::new("/test/dir"));
        assert!(result.is_ok());
    ...
    async fn test_fish_shell_command_special_handling() {
        let result = shell_command_builder::build_environment_capture_command("/usr/local/bin/fish", Path::new("/test/dir"));
        assert!(result.is_ok());
    ...
    async fn test_project_environment_caching_interface() {
        ...
        project_env.invalidate_directory_cache(Path::new("/test")).await;
    ...
    async fn test_lsp_environment_methods() {
        ...
        let lsp_env = project_env.get_lsp_environment(Path::new("/test/project")).await;
        assert!(lsp_env.is_ok());
        
        let env = lsp_env.unwrap();
        assert!(env.get("PATH").unwrap().contains("rust"));
        assert_eq!(env.get("CARGO_HOME"), Some(&"/Users/test/.cargo".to_string()));
        ...
        let merged_env = project_env.get_lsp_environment_with_overrides(Path::new("/test/project"), overrides).await;
